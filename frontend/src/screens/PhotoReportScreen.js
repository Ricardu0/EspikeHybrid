...
import {
    ...
    StyleSheet, ScrollView, Platform, Modal, Dimensions, FlatList,
    Animated, StatusBar, TouchableWithoutFeedback, RefreshControl,
} from 'react-native';
...
function PhotoCard({ item, onPress, distKm }) {
    ...
    useEffect(() => {
        if (!label && !isNaN(lat) && !isNaN(lng)) {
            reverseGeocode(lat, lng).then(p => setLabel(p || `${lat.toFixed(3)}, ${lng.toFixed(3)}`));
        }
    }, []);

    return (
        <TouchableOpacity onPress={onPress} activeOpacity={0.8} style={[styles.card, { width: CARD_W }]}>
...
function ExpandedModal({ item, onClose }) {
    ...
    const close = () => {
        Animated.timing(fade, { toValue: 0, duration: 160, useNativeDriver: true }).start(onClose);
    };
                        ...
                        <View style={styles.modalBox}>
                            ...
                            <View style={styles.modalInfo}>
                                <Text style={styles.modalDesc} numberOfLines={2}>
                                    {isGeneric(item.description) ? '📍 Reporte comunitário' : `📍 ${item.description}`}
                                </Text>
...
function UploadSheet({ userLocation, onSuccess, onClose }) {
    ...
    const dismiss = (cb) => {
        Animated.timing(slideY, { toValue: SCREEN_H, duration: 220, useNativeDriver: true }).start(cb);
    };
    ...
    const handleSubmit = async () => {
        ...
        if (!userLocation) return Alert.alert('Atenção', 'Aguarde a localização ser detectada.');
        ...
        try {
            ...
            if (Platform.OS === 'web') {
                ...
                formData.append('photo', { uri, name: `report_${Date.now()}.jpg`, type: 'image/jpeg' });
            }

            formData.append('latitude',    String(userLocation.lat));
            formData.append('longitude',   String(userLocation.lng));
            formData.append('description', userLocation.placeName || 'Reporte via app Espike');
                        ...
                        <Animated.View style={[styles.sheet, { transform: [{ translateY: slideY }] }]}>
                            ...
                            {image ? (
                                <TouchableOpacity onPress={pickImage} activeOpacity={0.85}>
                                    ...
                                    <Text style={styles.changePhoto}>Toque para trocar a foto</Text>
                                </TouchableOpacity>
                            ) : (
                                <TouchableOpacity style={styles.sheetCamera} onPress={pickImage}>
                            ...
                            <TouchableOpacity
                                style={[styles.sendBtn, (!image || uploading) && styles.sendBtnDisabled]}
...
export default function PhotoReportScreen() {
    ...
    useEffect(() => {
        (async () => {
            ...
            const placeName = await reverseGeocode(lat, lng) ?? `${lat.toFixed(4)}, ${lng.toFixed(4)}`;
    ...
    const photos = tab === 'nearby' && userLocation
        ? allPhotos
            .map(p => ({
                ...p,
                _dist: haversineKm(
        ...
        <View style={styles.screen}>
            ...
            <View style={styles.header}>
                <View style={{ flex: 1, marginRight: 12 }}>
                    ...
                    {locStatus === 'ok' && (
                        <Text style={styles.headerSub} numberOfLines={1}>📍 {userLocation?.placeName}</Text>
                    )}
                    {locStatus === 'denied' && (
                        <Text style={styles.headerSubWarn}>⚠️ GPS sem permissão</Text>
                    )}
                </View>
                <TouchableOpacity style={styles.fabSmall} onPress={() => setShowUpload(true)}>
            ...
            {/* Grid de fotos */}
            {loading ? (
                <ActivityIndicator style={{ marginTop: 48 }} size="large" color="#007AFF" />
            ) : (
                <FlatList
                    data={photos}
                    ...
                    refreshControl={
                        <RefreshControl refreshing={refreshing} onRefresh={() => fetchPhotos(true)} />
                    }
                    ListEmptyComponent={
                        <View style={styles.empty}>
                            {tab === 'nearby' && locStatus === 'denied'
                                ? <Text style={styles.emptyText}>Ative o GPS para ver reportes próximos.</Text>
                                : tab === 'nearby'
                                    ? <Text style={styles.emptyText}>Nenhum reporte num raio de {NEARBY_KM} km de você.</Text>
                                    : <Text style={styles.emptyText}>Nenhum reporte ainda. Seja o primeiro!</Text>
                            }
                        </View>
                    }
                />
            ...
            {showUpload && (
                <UploadSheet
                    ...
                    onSuccess={() => { setShowUpload(false); fetchPhotos(true); }}
...
const styles = StyleSheet.create({
    ...
    header:           { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 16, paddingTop: 16, paddingBottom: 12, backgroundColor: '#fff', borderBottomWidth: 1, borderBottomColor: '#eee' },
    ...
    tabs:             { flexDirection: 'row', backgroundColor: '#fff', paddingHorizontal: 16, borderBottomWidth: 1, borderBottomColor: '#eee' },
    ...
    card:             { backgroundColor: '#fff', borderRadius: 12, overflow: 'hidden', shadowColor: '#000', shadowOpacity: 0.07, shadowRadius: 6, shadowOffset: { width: 0, height: 2 }, elevation: 2 },
    cardImg:          { width: '100%', aspectRatio: 1 },
    cardFooter:       { padding: 8 },
    cardLabel:        { fontSize: 11, color: '#555', lineHeight: 15 },
    distBadge:        { position: 'absolute', top: 8, right: 8, backgroundColor: 'rgba(0,0,0,0.55)', borderRadius: 10, paddingHorizontal: 7, paddingVertical: 3 },
    distBadgeText:    { color: '#fff', fontSize: 11, fontWeight: '600' },

    empty:            { flex: 1, alignItems: 'center', paddingTop: 60 },
    emptyText:        { color: '#aaa', fontSize: 14, textAlign: 'center', paddingHorizontal: 32 },

    fab:              { position: 'absolute', right: 20, bottom: 28, width: 56, height: 56, borderRadius: 28, backgroundColor: '#007AFF', alignItems: 'center', justifyContent: 'center', shadowColor: '#007AFF', shadowOpacity: 0.4, shadowRadius: 8, shadowOffset: { width: 0, height: 4 }, elevation: 6 },
    fabText:          { color: '#fff', fontSize: 28, lineHeight: 34 },
    fabSmall:         { backgroundColor: '#007AFF', borderRadius: 8, paddingHorizontal: 14, paddingVertical: 8 },
    ...
    modalDesc:        { color: '#fff', fontSize: 16, fontWeight: '600', marginBottom: 6 },
    modalMeta:        { color: '#aaa', fontSize: 13, marginBottom: 3 },
    modalClose:       { alignSelf: 'flex-end', marginBottom: 12, width: 36, height: 36, borderRadius: 18, backgroundColor: 'rgba(255,255,255,0.15)', alignItems: 'center', justifyContent: 'center' },
    modalCloseText:   { color: '#fff', fontSize: 16 },

    sheetOverlay:     { flex: 1, backgroundColor: 'rgba(0,0,0,0.4)', justifyContent: 'flex-end' },
    sheet:            { backgroundColor: '#fff', borderTopLeftRadius: 20, borderTopRightRadius: 20, paddingHorizontal: 20, paddingBottom: 40, paddingTop: 12 },
    sheetHandle:      { width: 40, height: 4, backgroundColor: '#ddd', borderRadius: 2, alignSelf: 'center', marginBottom: 16 },
    sheetTitle:       { fontSize: 18, fontWeight: '700', color: '#111', marginBottom: 14 },
    locationRow:      { flexDirection: 'row', alignItems: 'center', backgroundColor: '#f0f7ff', borderRadius: 8, padding: 10, marginBottom: 14 },
    locationPin:      { fontSize: 16, marginRight: 6 },
    locationText:     { fontSize: 13, color: '#007AFF', fontWeight: '500', flex: 1 },
    sheetCamera:      { height: 160, borderRadius: 12, borderWidth: 2, borderColor: '#e0e0e0', borderStyle: 'dashed', alignItems: 'center', justifyContent: 'center', marginBottom: 14, backgroundColor: '#fafafa' },
    sheetCameraIcon:  { fontSize: 36, marginBottom: 8 },
    sheetCameraLabel: { fontSize: 15, color: '#888' },
    sheetPreview:     { width: '100%', height: 200, borderRadius: 12, marginBottom: 6 },
    changePhoto:      { textAlign: 'center', fontSize: 12, color: '#999', marginBottom: 14 },
    sendBtn:          { backgroundColor: '#007AFF', borderRadius: 12, paddingVertical: 16, alignItems: 'center' },
